...
connection is online. The selector also contains `No connection`. Selecting a
target only binds the Console; it does not connect. Running SQL establishes or
reuses that connection, while `No connection` keeps the Console available for
offline editing and formatting.
...
`y` copies the current cell immediately. `Y` copies the current row as TSV.
