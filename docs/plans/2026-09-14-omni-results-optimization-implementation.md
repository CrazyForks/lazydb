...
- Task 1：新增重复 Catalog ID 的状态回归测试；先确认红灯，再由 Task 2 修复。通过 `cargo test --lib model::omni::tests` 及 Omni 搜索/导航测试。
- Task 2：`OmniState::set_items` 按稳定 `OmniItemId` 去重。状态、搜索、导航测试通过。
- Task 3：新增 `CatalogSearchObjectScope`；Omni 使用 `RelationsOnly`，Agent/既有调用者使用 `AllObjects`。`cargo check --all-targets --all-features` 通过。
- Task 4：PostgreSQL、MySQL/MariaDB、SQLite、SQL Server 适配器及 app 防御层接入 relation-only 过滤；SQLite 26 项测试、范围测试、Omni provider/search 测试通过。
- Task 5：`OmniLocation` 接入本地和远程 relation 候选，统一 profile/database/schema 元信息；全目标编译通过。
- Task 6：Omni 行改为对象左侧、连接图标和元信息右侧布局，支持显示宽度截断、三种图标模式及 `[open]` success 样式。Omni/UI 定向测试通过。
- Task 7：`cargo +1.94.0 fmt --all -- --check` 通过；`cargo +1.94.0 clippy --all-targets --all-features -- -D warnings` 通过；Omni 全链路测试全部通过。
- 全量 `cargo +1.94.0 test --all-targets --all-features` 中，Omni/Catalog/SQLite/SQL Server 相关测试通过；`tests/ui_render.rs` 有两个与本变更无关的既有失败：`data_grid_keeps_null_muted_on_the_selected_row`、`help_overlay_is_contextual`。
